...
strategy = "Supertrend Python Batch-2"
symbols = ["HDFCBANK", "SBIN", "EICHER", "KALYANKJIL", "AXISBANK", "DRREDDY", "HINDUNILVR", "HDFCLIFE"]
exchange = "NSE"
product = "MIS"
quantity = 5
atr_period = 12
...
stop_loss_pct = 0.4       # Temporarily reduced SL for testing - 1% SL
target_pct = 1            # Temporarily reduced SL for testing - 1.5% profit target
trailing_sl_pct = 0.2     # Temporarily lowered for testing     # Start trailing SL when in profit; trail at 0.5%
...
# Logging
LOG_FILE = f"logs/ST_B2_{datetime.now().strftime('%Y-%m-%d')}.txt"
TRADE_LOG = f"logs/ST_B2_{datetime.now().strftime('%Y-%m-%d')}.csv"
...
if __name__ == "__main__":
    print("Starting Amar's Supertrend Batch-2 Multi-Stock Strategy...")
    send_telegram(f"✅ Amar's Supertrend Batch-2 Multi-Stock strategy started in {mode.upper()} mode.")
    log_message(f"Amar's Supertrend Batch-2 Strategy started in {mode.upper()} mode.")
    def graceful_exit(signum, frame):
        print("Graceful shutdown requested... Exiting strategy.")
        send_telegram("🛑 Supertrend Batch-2 Strategy stopped gracefully.")
